...
c
c     vext(y,1,i) cext(y,1,1-3,i) the minimum comp i in plane y, its x,y,z coord
c     vext(y,2,i) cext(y,2,1-3,i) the maximum comp i in plane y, its x,y,z coord
c     i=1 ... 6 : u,v,w,omegax,omegay,omegaz
c
      do x=1,nxp/2
         xc1(x)=float(2*x-1-nxp/2-1)/float(nxp)*xl+xs
         xc2(x)=float(2*x-nxp/2-1)/float(nxp)*xl+xs
      end do
      do i=1,3
     ...
     &           cext(y,1,i+3,2),om2r(1,1,1,i),om2i(1,1,1,i),xc1,xc2,zl)
            call plnmax(vext(y,2,i+3),y1,cext(y,2,i+3,1),
     &           cext(y,2,i+3,2),om2r(1,1,1,i),om2i(1,1,1,i),xc1,xc2,zl)
         end do
      end do
